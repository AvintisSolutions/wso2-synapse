...
public class VFSTransportListener extends AbstractPollingTransportListener<PollTableEntry>
    ...
    private static final HashMap<String, String> PAIRWISE_TRANS = new HashMap<String, String>();
                        ...
                        if(entry.getFileNamePattern()!=null &&
                            ...
                            if(entry.hasRequiredFileNamePatterns()){
                                ...
                                if(hasPatternMissing) {
                                    ...
                                    // If same transaction feature is enabled
                                    if(entry.isMultiFileReadSameTransactionEnabled()) {
                                        String matchedPattern = VFSUtils.getMatchedPattern(requiredPatternReplacedList, children, child.getName().getBaseName());
                                        if(matchedPattern != null) {
                                            // Pattern registered, simply get stored UUID
                                            if(PAIRWISE_TRANS.get(matchedPattern) != null) {
                                                transactionId = PAIRWISE_TRANS.get(matchedPattern);
                                                PAIRWISE_TRANS.remove(matchedPattern);
                                            } else {
                                                // Nothing registered, init transactions list  
                                                transactionId = UUID.randomUUID().toString();
                                                for (String requiredPattern : requiredPatternReplacedList) {
                                                    // Exclude current file which is already managed
                                                    if(!requiredPattern.equals(matchedPattern)) {
                                                        PAIRWISE_TRANS.put(requiredPattern, transactionId);
                                                    }
                                                }
                                            }
                                        }
